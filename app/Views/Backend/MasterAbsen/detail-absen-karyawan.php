...
<script type="text/javascript">
    // Validasi jika data lokasi tidak null atau kosong
        var lokasiKantor = L.latLng(<?= $data_absen['lokasi']; ?>);
        var lokasiUser = L.latLng(<?= $data_absen['lokasi_masuk']; ?>);

        var masuk = L.map('masuk').setView(lokasiKantor, 18);

        L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
        }).addTo(masuk);

        var circle = L.circle(lokasiKantor, {
            color: 'red',
            fillColor: 'blue',
            fillOpacity: 0.5,
        ...
        L.marker(lokasiUser).addTo(masuk)
        L.marker(lokasiKantor, {
            icon: LokasiIcon
        }).addTo(masuk)
            .bindPopup('<?= $data_absen['nama_lokasi']; ?>')
            .openPopup();


        var lokasiKeluar = L.latLng(<?= $data_absen['lokasi_keluar']; ?>);
        var keluar = L.map('keluar').setView(lokasiKeluar, 15);

        L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
        }).addTo(keluar);

        var circleKeluar = L.circle(lokasiKantor, {
            color: 'red',
            fillColor: 'blue',
            fillOpacity: 0.5,
            radius: <?= isset($data_absen['radius']) ? $data_absen['radius'] : 100; ?>
        }).addTo(keluar);

        L.marker(lokasiKeluar).addTo(keluar)
            .bindPopup('Lokasi Absen Pulang: <?= $data_absen['lokasi_keluar']; ?>')
            .openPopup();
</script>
